...
func (self *Ptfs) Rename(oldpath string, newpath string) int {
	oldpath = filepath.Join(self.root, oldpath)
	newpath = filepath.Join(self.root, newpath)
	return errno(syscall.Rename(oldpath, newpath))
}

func (self *Ptfs) Chmod(path string, mode uint32) int {
	path = filepath.Join(self.root, path)
	return errno(syscall.Chmod(path, mode))
}

func (self *Ptfs) Chown(path string, uid uint32, gid uint32) int {
	path = filepath.Join(self.root, path)
	return errno(syscall.Chown(path, int(uid), int(gid)))
}

func (self *Ptfs) Utimens(path string, tmsp1 []fuse.Timespec) int {
	path = filepath.Join(self.root, path)
...
func (self *Ptfs) Create(path string, mode uint32) (int, uint64) {
	path = filepath.Join(self.root, path)
...
func (self *Ptfs) Open(path string, flags int) (int, uint64) {
	path = filepath.Join(self.root, path)
...
func (self *Ptfs) Read(path string, buff []byte, ofst int64, fh uint64) int {
...
func (self *Ptfs) Write(path string, buff []byte, ofst int64, fh uint64) int {
...
func (self *Ptfs) Release(path string, fh uint64) int {
	return errno(syscall.Close(int(fh)))
}

func (self *Ptfs) Fsync(path string, datasync bool, fh uint64) int {
	return errno(syscall.Fsync(int(fh)))
}

func (self *Ptfs) Opendir(path string) (int, uint64) {
	path = filepath.Join(self.root, path)
...
func (self *Ptfs) Releasedir(path string, fh uint64) int {
	return errno(syscall.Close(int(fh)))
}
